...
// Instrukcja konfiguracji w SETUP.md
// ============================================
let scarfStock = {};
    ...
    beige: [
        {
            ...
            serialNumber: '01',
            color: 'Uniesienie I',
            priceOriginal: '450 PLN',
            pricePrint: '280 PLN',
            material: '100% jedwab habotai',
            size: '90 × 90 cm',
...
function openScarfModal(colorKey, scarfIndex) {
    ...
    // Przycisk zamówienia
    // Type btn — zmiana ceny
    document.querySelectorAll('.type-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            document.querySelectorAll('.type-btn').forEach(b => b.classList.remove('active'));
            this.classList.add('active');
            const type = this.dataset.type;
            const colorKey = sessionStorage.getItem('selectedColor');
            const idx = sessionStorage.getItem('selectedIndex');
            if (colorKey && idx !== null) {
                const scarf = collections[colorKey][idx];
                const price = type === 'original' ? scarf.priceOriginal : scarf.pricePrint;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // EmailJS config (te same klucze co w checkout.js)
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async function(e) {
            ...
            if (typeof emailjs !== 'undefined') {
                try {
                    await emailjs.send(EJS_SERVICE_ID, EJS_TEMPLATE_MSG, {
